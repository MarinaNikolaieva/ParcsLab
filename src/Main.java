...
public class Main implements AM {
    ...
    public static boolean solve(AMInfo info, BigInteger number, BigInteger iters, int threads){
        //NEEDED finish this part, this is where the algo is
        List<Integer> reses = new ArrayList<Integer>();
        List<point> points = new ArrayList<point>();
        List<channel> channels = new ArrayList<channel>();
        
        BigInteger threadNum = BigInteger.valueOf(threads);
        BigInteger iterPerThread = iters.divide(threadNum);
        
        for (int i = 0; i < threads; i++){
            points.add(info.createPoint());
            channels.add(points.get(i).createChannel());
            points.get(i).execute("Algorithm");
            channels.get(i).write(number.longValue());
            channels.get(i).write(iterPerThread.longValue());
        }
